...
class Wallet{
    ...
    createTransaction(recipient, amount, blockchain, transactionPool){
        ...
        if(amount > this.balance){
            console.log(`Amount: ${amount} exceeds the current wallet balance: ${this.balance}`);
            return;
        }
    ...
    calculateBalance(blockchain){
        ...
        if(walletInputTransactions.length > 0){
            const recentInputTransaction= walletInputTransactions.reduce((previous, current) => previous.input.timestamp > current.input.timestamp ? previous : current);
